...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full text-white transition-colors">
      <div className="container mx-auto py-0">
        <div className={`${scrolled ? "bg-[#0B0F1A]/30 border-white/5 backdrop-blur" : "bg-transparent border-transparent"} flex h-14 items-center justify-between rounded-full px-4 md:px-6`}>
        <Link to="/" className="flex items-center gap-2">
          <Logo />
        </Link>
        <div className="flex items-center gap-6">
          <nav className="hidden md:flex items-center gap-6 text-sm">
            <Link to="/dashboard" className="hover:text-white/90 text-white/80">Dashboard</Link>
